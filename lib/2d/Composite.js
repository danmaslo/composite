...
class Composite {
    ...
    saveTxt(filename) {
        ...
        // general info
        txt = txt + "# WIDTH: " + this._width + "\n";
        txt = txt + "# HEIGHT: " + this._height + "\n";
        txt = txt + "# BOUNDS: " + this._bounds + "\n";

        for (var i = 0; i < this._objects.length; i++) {
            txt = txt + this._objects[i].getCenter().getX() + "\t" + this._objects[i].getCenter().getY() + "\t" + this._objects[i].getRadius() + "\n";
        }

        Fs.writeFileSync(filename + ".txt", txt, function (err) {
			if (err) return console.log(err);
		});
    }
    ...
    loadTxt(filename) {
        ...
        // general info
        var width = lines[0].toString().split(" ");
        var height = lines[1].toString().split(" ");
        var bounds = lines[2].toString().split(" ");

        this._width = parseInt(width[2]);
        this._height = parseInt(height[2]);
        this._bounds = parseInt(bounds[2]);

        for (var i = 3; i < lines.length; i++) {
            var coords = lines[i].split("\t");

            this._objects[i - 3] = new Circle(new Vector(coords[0], coords[1]), coords[2]);
        }
    }
	...
	*/
    ...
    checkRadiusDistributions(initObj) {
        var sum = 0;
    ...
    export(initObj) {
        if ((initObj.type != null) && (initObj.file != null)) {
            var type = initObj.type;
            var file = initObj.file;

            // SVG export
            if (type == Composite.EXPORT_SVG) {
                this.saveSvg(file);
            }
        }
        else {
            console.log("missing TYPE or FILE argument in composite.export() method");
            process.exit();
        }
    }
    ...
    generate(initObj) {
        if ((initObj.until != null) && (initObj.particles != null)) {
            ...
            var probabilities = this.checkRadiusDistributions(initObj);
